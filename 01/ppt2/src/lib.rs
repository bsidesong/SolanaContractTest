...
const DATA_SIZE: usize = SCORE_SIZE + (HISTORY_SIZE * RECORD_SIZE) + 1 + 8; // +1 byte for history_len, +8 bytes for plays_count
const RECIPIENT_PUBKEY: Pubkey = Pubkey::new_from_array([19, 140, 160, 164, 193, 106, 179, 126, 191, 167, 19, 107, 100, 104, 174, 106, 163, 109, 18, 169, 191, 178, 104, 176, 104, 176, 104, 176, 104, 176, 104, 176]); // DEr8Y8GG19SSXHBDMcFVvxwrcZxFcihsm3CrQZtkrkbH
const PLAYS_PER_PAYMENT: u8 = 5;
const PAYMENT_AMOUNT: u64 = 10000; // 0.01 SOL = 10_000 lamports
...
pub fn process_instruction(
    ...
    if pda_account.lamports() == 0 || pda_account.data_len() < DATA_SIZE {
        ...
        data[SCORE_SIZE + 1 + HISTORY_SIZE * RECORD_SIZE .. SCORE_SIZE + 1 + HISTORY_SIZE * RECORD_SIZE + 8].copy_from_slice(&0u64.to_le_bytes()); // plays_count = 0
    ...
    let mut plays_count = u64::from_le_bytes(data[SCORE_SIZE + 1 + HISTORY_SIZE * RECORD_SIZE .. SCORE_SIZE + 1 + HISTORY_SIZE * RECORD_SIZE + 8].try_into().unwrap());

    // Payment logic
    if plays_count % PLAYS_PER_PAYMENT as u64 == 0 {
        if payer.lamports() < PAYMENT_AMOUNT {
        ...
        solana_program::program::invoke(
            ...
            &[payer.clone(), recipient_account.clone()],
        )?;
    }

    plays_count = plays_count.checked_add(1).ok_or(ProgramError::Custom(1))?;
    data[SCORE_SIZE + 1 + HISTORY_SIZE * RECORD_SIZE .. SCORE_SIZE + 1 + HISTORY_SIZE * RECORD_SIZE + 8].copy_from_slice(&plays_count.to_le_bytes());
